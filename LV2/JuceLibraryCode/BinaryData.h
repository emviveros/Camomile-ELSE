/* =========================================================================================

   This is an auto-generated file: Any edits you make may be overwritten!

*/

#pragma once

namespace BinaryData
{
    extern const char*   above_pd;
    const int            above_pdSize = 419;

    extern const char*   add_pd;
    const int            add_pdSize = 591;

    extern const char*   All_objects_pd;
    const int            All_objects_pdSize = 15843;

    extern const char*   allpass_filt_pd;
    const int            allpass_filt_pdSize = 2425;

    extern const char*   allpass_unit_pd;
    const int            allpass_unit_pdSize = 468;

    extern const char*   amean_pd;
    const int            amean_pdSize = 1211;

    extern const char*   any2symbol_pd;
    const int            any2symbol_pdSize = 1636;

    extern const char*   autotune_pd;
    const int            autotune_pdSize = 2935;

    extern const char*   autotune2_pd;
    const int            autotune2_pdSize = 2511;

    extern const char*   avg_pd;
    const int            avg_pdSize = 681;

    extern const char*   bangdiv_pd;
    const int            bangdiv_pdSize = 1887;

    extern const char*   batch_rec_pd;
    const int            batch_rec_pdSize = 3903;

    extern const char*   batch_write_pd;
    const int            batch_write_pdSize = 3622;

    extern const char*   bin_shift_pd;
    const int            bin_shift_pdSize = 2386;

    extern const char*   biplot_pd;
    const int            biplot_pdSize = 9304;

    extern const char*   bl_imp_pd;
    const int            bl_imp_pdSize = 759;

    extern const char*   bl_imp2_pd;
    const int            bl_imp2_pdSize = 936;

    extern const char*   bl_saw_pd;
    const int            bl_saw_pdSize = 767;

    extern const char*   bl_saw2_pd;
    const int            bl_saw2_pdSize = 770;

    extern const char*   bl_sine_pd;
    const int            bl_sine_pdSize = 769;

    extern const char*   bl_square_pd;
    const int            bl_square_pdSize = 942;

    extern const char*   bl_tri_pd;
    const int            bl_tri_pdSize = 774;

    extern const char*   bl_vsaw_pd;
    const int            bl_vsaw_pdSize = 885;

    extern const char*   bl_wavetable_pd;
    const int            bl_wavetable_pdSize = 815;

    extern const char*   bpbank_pd;
    const int            bpbank_pdSize = 2168;

    extern const char*   bpclone_pd;
    const int            bpclone_pdSize = 1000;

    extern const char*   bpm_pd;
    const int            bpm_pdSize = 623;

    extern const char*   brickwall_pd;
    const int            brickwall_pdSize = 10627;

    extern const char*   car2pol_pd;
    const int            car2pol_pdSize = 995;

    extern const char*   car2pol_pd2;
    const int            car2pol_pd2Size = 573;

    extern const char*   cents2frac_pd;
    const int            cents2frac_pdSize = 2866;

    extern const char*   cents2scale_pd;
    const int            cents2scale_pdSize = 776;

    extern const char*   chorus_pd;
    const int            chorus_pdSize = 1364;

    extern const char*   circle_pd;
    const int            circle_pdSize = 34520;

    extern const char*   clock_pd;
    const int            clock_pdSize = 1901;

    extern const char*   combine_pd;
    const int            combine_pdSize = 1352;

    extern const char*   compress_pd;
    const int            compress_pdSize = 2288;

    extern const char*   conv_pd;
    const int            conv_pdSize = 9533;

    extern const char*   convpartition_pd;
    const int            convpartition_pdSize = 3939;

    extern const char*   count_pd;
    const int            count_pdSize = 9202;

    extern const char*   crusher_pd;
    const int            crusher_pdSize = 1009;

    extern const char*   db2lin_pd;
    const int            db2lin_pdSize = 500;

    extern const char*   db2lin_pd2;
    const int            db2lin_pd2Size = 430;

    extern const char*   dec2frac_pd;
    const int            dec2frac_pdSize = 2817;

    extern const char*   dec2hex_pd;
    const int            dec2hex_pdSize = 748;

    extern const char*   display_pd;
    const int            display_pdSize = 12552;

    extern const char*   display_pd2;
    const int            display_pd2Size = 5207;

    extern const char*   drum_seq_pd;
    const int            drum_seq_pdSize = 27555;

    extern const char*   drunkard_pd;
    const int            drunkard_pdSize = 2427;

    extern const char*   drunkard_pd2;
    const int            drunkard_pd2Size = 3134;

    extern const char*   duck_pd;
    const int            duck_pdSize = 2475;

    extern const char*   e_pd;
    const int            e_pdSize = 776;

    extern const char*   echo_rev_pd;
    const int            echo_rev_pdSize = 2260;

    extern const char*   echo_unit_pd;
    const int            echo_unit_pdSize = 482;

    extern const char*   envelope_pd;
    const int            envelope_pdSize = 113677;

    extern const char*   eqdiv_pd;
    const int            eqdiv_pdSize = 2175;

    extern const char*   euclid_pd;
    const int            euclid_pdSize = 1487;

    extern const char*   expand_pd;
    const int            expand_pdSize = 1885;

    extern const char*   flanger_pd;
    const int            flanger_pdSize = 1427;

    extern const char*   frac2cents_pd;
    const int            frac2cents_pdSize = 667;

    extern const char*   frac2dec_pd;
    const int            frac2dec_pdSize = 615;

    extern const char*   frac2ratio_pd;
    const int            frac2ratio_pdSize = 615;

    extern const char*   free_rev_pd;
    const int            free_rev_pdSize = 20955;

    extern const char*   freeze_osc_clone_pd;
    const int            freeze_osc_clone_pdSize = 1045;

    extern const char*   freeze_pd;
    const int            freeze_pdSize = 1482;

    extern const char*   gain_pd;
    const int            gain_pdSize = 9741;

    extern const char*   gain2_pd;
    const int            gain2_pdSize = 10165;

    extern const char*   gatehold_pd;
    const int            gatehold_pdSize = 698;

    extern const char*   gmean_pd;
    const int            gmean_pdSize = 1791;

    extern const char*   grain_live_grain_pd;
    const int            grain_live_grain_pdSize = 44803;

    extern const char*   grain_live_pd;
    const int            grain_live_pdSize = 3730;

    extern const char*   grain_sampler_grain_pd;
    const int            grain_sampler_grain_pdSize = 44242;

    extern const char*   grain_sampler_pd;
    const int            grain_sampler_pdSize = 2622;

    extern const char*   grain_synth_grain_pd;
    const int            grain_synth_grain_pdSize = 43201;

    extern const char*   grain_synth_pd;
    const int            grain_synth_pdSize = 2592;

    extern const char*   gran_player_pd;
    const int            gran_player_pdSize = 13354;

    extern const char*   gran_pd;
    const int            gran_pdSize = 4857;

    extern const char*   graph_pd;
    const int            graph_pdSize = 10300;

    extern const char*   group_pd;
    const int            group_pdSize = 2271;

    extern const char*   hann_pd;
    const int            hann_pdSize = 1202;

    extern const char*   hex2dec_pd;
    const int            hex2dec_pdSize = 2081;

    extern const char*   hip_bw_pd;
    const int            hip_bw_pdSize = 3317;

    extern const char*   impulse_pd;
    const int            impulse_pdSize = 3686;

    extern const char*   insert_pd;
    const int            insert_pdSize = 1477;

    extern const char*   interpolate_pd;
    const int            interpolate_pdSize = 3544;

    extern const char*   iterate_pd;
    const int            iterate_pdSize = 1930;

    extern const char*   lastvalue_pd;
    const int            lastvalue_pdSize = 1284;

    extern const char*   lcm_pd;
    const int            lcm_pdSize = 2917;

    extern const char*   lfnoise_pd;
    const int            lfnoise_pdSize = 2431;

    extern const char*   lfo_pd;
    const int            lfo_pdSize = 15833;

    extern const char*   lin2db_pd;
    const int            lin2db_pdSize = 373;

    extern const char*   lin2db_pd2;
    const int            lin2db_pd2Size = 337;

    extern const char*   lop_bw_pd;
    const int            lop_bw_pdSize = 3323;

    extern const char*   mag_pd;
    const int            mag_pdSize = 1142;

    extern const char*   mag_pd2;
    const int            mag_pd2Size = 852;

    extern const char*   markov_pd;
    const int            markov_pdSize = 8695;

    extern const char*   maxpeak_pd;
    const int            maxpeak_pdSize = 685;

    extern const char*   median_pd;
    const int            median_pdSize = 871;

    extern const char*   meter_pd;
    const int            meter_pdSize = 800;

    extern const char*   meter_pd2;
    const int            meter_pd2Size = 1530;

    extern const char*   meter2_pd;
    const int            meter2_pdSize = 2619;

    extern const char*   meter4_pd;
    const int            meter4_pdSize = 4813;

    extern const char*   meter8_pd;
    const int            meter8_pdSize = 9439;

    extern const char*   midi2freq_pd;
    const int            midi2freq_pdSize = 592;

    extern const char*   mix2_pd;
    const int            mix2_pdSize = 7879;

    extern const char*   mix4_pd;
    const int            mix4_pdSize = 14925;

    extern const char*   mono_pd;
    const int            mono_pdSize = 5614;

    extern const char*   mono_rev_pd;
    const int            mono_rev_pdSize = 24176;

    extern const char*   morph_pd;
    const int            morph_pdSize = 2713;

    extern const char*   morph_pd2;
    const int            morph_pd2Size = 1587;

    extern const char*   mov_avg_pd;
    const int            mov_avg_pdSize = 1055;

    extern const char*   ms2samps_pd;
    const int            ms2samps_pdSize = 685;

    extern const char*   ms2samps_pd2;
    const int            ms2samps_pd2Size = 390;

    extern const char*   mtx_ctl_pd;
    const int            mtx_ctl_pdSize = 18460;

    extern const char*   multi_vsl_pd;
    const int            multi_vsl_pdSize = 37638;

    extern const char*   multi_vsl_unit_pd;
    const int            multi_vsl_unit_pdSize = 3461;

    extern const char*   nmess_pd;
    const int            nmess_pdSize = 899;

    extern const char*   noisegate_pd;
    const int            noisegate_pdSize = 1090;

    extern const char*   norm_pd;
    const int            norm_pdSize = 979;

    extern const char*   note2pitch_pd;
    const int            note2pitch_pdSize = 2637;

    extern const char*   op_pd;
    const int            op_pdSize = 8111;

    extern const char*   order_pd;
    const int            order_pdSize = 1289;

    extern const char*   oscbank_pd;
    const int            oscbank_pdSize = 2264;

    extern const char*   oscbank2_pd;
    const int            oscbank2_pdSize = 2156;

    extern const char*   oscclone_pd;
    const int            oscclone_pdSize = 1039;

    extern const char*   oscclone2_pd;
    const int            oscclone2_pdSize = 949;

    extern const char*   out_pd;
    const int            out_pdSize = 14912;

    extern const char*   out1_pd;
    const int            out1_pdSize = 14489;

    extern const char*   out4_pd;
    const int            out4_pdSize = 14685;

    extern const char*   out8_pd;
    const int            out8_pdSize = 15302;

    extern const char*   output_pd;
    const int            output_pdSize = 11575;

    extern const char*   pan8_pd;
    const int            pan8_pdSize = 13283;

    extern const char*   pattern_pd;
    const int            pattern_pdSize = 6154;

    extern const char*   perlin_pd;
    const int            perlin_pdSize = 1939;

    extern const char*   phaser_pd;
    const int            phaser_pdSize = 1252;

    extern const char*   phasor_pd;
    const int            phasor_pdSize = 3741;

    extern const char*   pi_pd;
    const int            pi_pdSize = 905;

    extern const char*   pick_pd;
    const int            pick_pdSize = 2365;

    extern const char*   pimp_pd;
    const int            pimp_pdSize = 3933;

    extern const char*   ping_pong_pd;
    const int            ping_pong_pdSize = 1184;

    extern const char*   pipe2_pd;
    const int            pipe2_pdSize = 772;

    extern const char*   pitch_shift_pd;
    const int            pitch_shift_pdSize = 1553;

    extern const char*   pitch2note_pd;
    const int            pitch2note_pdSize = 7924;

    extern const char*   plate_rev_pd;
    const int            plate_rev_pdSize = 9731;

    extern const char*   play_file_pd;
    const int            play_file_pdSize = 22547;

    extern const char*   player_pd;
    const int            player_pdSize = 11481;

    extern const char*   pol2car_pd;
    const int            pol2car_pdSize = 362;

    extern const char*   pol2car_pd2;
    const int            pol2car_pd2Size = 367;

    extern const char*   presets_pd;
    const int            presets_pdSize = 15138;

    extern const char*   presets_send_clone_pd;
    const int            presets_send_clone_pdSize = 4140;

    extern const char*   pulse_pd;
    const int            pulse_pdSize = 5603;

    extern const char*   pvoc_freeze_pd;
    const int            pvoc_freeze_pdSize = 4837;

    extern const char*   pvoc_live_pd;
    const int            pvoc_live_pdSize = 7070;

    extern const char*   pvoc_player_pd;
    const int            pvoc_player_pdSize = 13789;

    extern const char*   pvoc_pd;
    const int            pvoc_pdSize = 4523;

    extern const char*   rampnoise_pd;
    const int            rampnoise_pdSize = 1784;

    extern const char*   rand_list_pd;
    const int            rand_list_pdSize = 746;

    extern const char*   randpulse_pd;
    const int            randpulse_pdSize = 5715;

    extern const char*   randpulse2_pd;
    const int            randpulse2_pdSize = 1308;

    extern const char*   range_hsl_pd;
    const int            range_hsl_pdSize = 24652;

    extern const char*   range_pd;
    const int            range_pdSize = 1033;

    extern const char*   rdelay_pd;
    const int            rdelay_pdSize = 927;

    extern const char*   rec_file_pd;
    const int            rec_file_pdSize = 5463;

    extern const char*   resonbank_pd;
    const int            resonbank_pdSize = 2181;

    extern const char*   resonbank2_pd;
    const int            resonbank2_pdSize = 2413;

    extern const char*   resonclone_pd;
    const int            resonclone_pdSize = 999;

    extern const char*   resonclone2_pd;
    const int            resonclone2_pdSize = 1117;

    extern const char*   retune_pd;
    const int            retune_pdSize = 2385;

    extern const char*   reverse_pd;
    const int            reverse_pdSize = 1033;

    extern const char*   rm_pd;
    const int            rm_pdSize = 848;

    extern const char*   rotate_pd;
    const int            rotate_pdSize = 1305;

    extern const char*   sample_pd;
    const int            sample_pdSize = 23034;

    extern const char*   samps2ms_pd;
    const int            samps2ms_pdSize = 683;

    extern const char*   samps2ms_pd2;
    const int            samps2ms_pd2Size = 391;

    extern const char*   scale2cents_pd;
    const int            scale2cents_pdSize = 592;

    extern const char*   scales_pd;
    const int            scales_pdSize = 8918;

    extern const char*   schmitt_pd;
    const int            schmitt_pdSize = 715;

    extern const char*   scramble_pd;
    const int            scramble_pdSize = 2147;

    extern const char*   sequencer_pd;
    const int            sequencer_pdSize = 5947;

    extern const char*   setdsp_pd;
    const int            setdsp_pdSize = 3302;

    extern const char*   slew_pd;
    const int            slew_pdSize = 3565;

    extern const char*   slew2_pd;
    const int            slew2_pdSize = 5493;

    extern const char*   slider2d_pd;
    const int            slider2d_pdSize = 35628;

    extern const char*   spectrograph_pd;
    const int            spectrograph_pdSize = 12961;

    extern const char*   stack_pd;
    const int            stack_pdSize = 6500;

    extern const char*   status_pd;
    const int            status_pdSize = 924;

    extern const char*   stepnoise_pd;
    const int            stepnoise_pdSize = 1492;

    extern const char*   stereo_rev_pd;
    const int            stereo_rev_pdSize = 42549;

    extern const char*   stream_pd;
    const int            stream_pdSize = 1400;

    extern const char*   stretch_shift_pd;
    const int            stretch_shift_pdSize = 4343;

    extern const char*   sum_pd;
    const int            sum_pdSize = 613;

    extern const char*   sysrt_in_pd;
    const int            sysrt_in_pdSize = 491;

    extern const char*   sysrt_out_pd;
    const int            sysrt_out_pdSize = 677;

    extern const char*   tempo_pd;
    const int            tempo_pdSize = 3574;

    extern const char*   timed_gate_pd;
    const int            timed_gate_pdSize = 1062;

    extern const char*   tremolo_pd;
    const int            tremolo_pdSize = 1280;

    extern const char*   trig2bang_pd;
    const int            trig2bang_pdSize = 330;

    extern const char*   trig2bang_pd2;
    const int            trig2bang_pd2Size = 333;

    extern const char*   unite_pd;
    const int            unite_pdSize = 2400;

    extern const char*   vibrato_pd;
    const int            vibrato_pdSize = 1282;

    extern const char*   vocoder_band_clone_pd;
    const int            vocoder_band_clone_pdSize = 757;

    extern const char*   vocoder_pd;
    const int            vocoder_pdSize = 2268;

    extern const char*   CreditsAU;
    const int            CreditsAUSize = 479;

    extern const char*   CreditsLV2;
    const int            CreditsLV2Size = 624;

    extern const char*   CreditsVST;
    const int            CreditsVSTSize = 537;

    extern const char*   DejaVuSansMono_ttf;
    const int            DejaVuSansMono_ttfSize = 340712;

    extern const char*   copy_png;
    const int            copy_pngSize = 4098;

    extern const char*   flower_center_png;
    const int            flower_center_pngSize = 3299;

    extern const char*   flower_petals_png;
    const int            flower_petals_pngSize = 22287;

    extern const char*   garbage_png;
    const int            garbage_pngSize = 5113;

    extern const char*   reload_png;
    const int            reload_pngSize = 8380;

    extern const char*   settings_png;
    const int            settings_pngSize = 12539;

    // Number of elements in the namedResourceList and originalFileNames arrays.
    const int namedResourceListSize = 212;

    // Points to the start of a list of resource names.
    extern const char* namedResourceList[];

    // Points to the start of a list of resource filenames.
    extern const char* originalFilenames[];

    // If you provide the name of one of the binary resource variables above, this function will
    // return the corresponding data and its size (or a null pointer if the name isn't found).
    const char* getNamedResource (const char* resourceNameUTF8, int& dataSizeInBytes);

    // If you provide the name of one of the binary resource variables above, this function will
    // return the corresponding original, non-mangled filename (or a null pointer if the name isn't found).
    const char* getNamedResourceOriginalFilename (const char* resourceNameUTF8);
}
